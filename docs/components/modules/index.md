...
A module has one exact package specifier, optional discovery description, and a materializer that yields a physical package root. `packageModule` links existing runtime JavaScript and declarations without interpreting the package's protocol. This is the complete integration for local libraries and network clients, including generated Protobuf/Connect clients: package code creates its own connection inside each fresh subprocess.

## Provided APIs

- `Module` — `{ specifier, description?, materialize(context) }`; materialization returns a package root that remains valid for the enclosing operation. The context provides an operation-local package location for materializers that need to prepare an ordinary package. The result contains only `packageRoot`; there is no parent-callback facility.
- `packageModule({ specifier, root, description? }): Module` — validates the exact package specifier, resolves `root`, and links an existing readable package directory without copying it. Its `package.json` exports, runtime JavaScript, and declarations define all behavior, including subpaths. `getTypes` rejects the package or subpath when TypeScript cannot resolve a package-owned `.d.ts`, `.d.mts`, or `.d.cts` entrypoint.
...
4. An operation snapshot invokes each materializer once, validates its package root, and links it at the exact specifier below the operation's `node_modules`.
