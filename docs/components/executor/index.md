...
- `getInstructions(options?: InstructionsOptions)` — `{ timeoutMs?, maxOutputBytes? }` makes the deterministic Markdown state the effective cap (always; the default when omitted) and the deadline (only when supplied) in a `## Limits` section, so the model reads the limits the harness enforces.
